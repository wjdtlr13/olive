<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<%@ page trimDirectiveWhitespaces="true" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>

<div class="col text-center"><img src="assets/img/security-guard.png"></div>
    <div class="col text-center">
        <p>올리브네 관리실</p>
    </div>
    <div class="row d-flex justify-content-center">
        <div class="col-auto"><button class="btn btn-secondary" type="button">회원 정보</button></div>
        <div class="col-auto"><button class="btn btn-secondary" type="button">신고 회원</button></div>
    </div>
    <div class="row d-flex justify-content-center">
        <div class="col-auto"><button class="btn btn-secondary" type="button">게시글 승인</button></div>
        <div class="col-auto"><button class="btn btn-secondary" type="button">미션 승인</button></div>
        <div class="col-auto"><button class="btn btn-secondary" type="button">주민센터 등록</button></div>
    </div>